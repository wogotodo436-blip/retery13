...
function shortenText(text: string): string {
  ...
  const replacements: Array<{ pattern: RegExp; replacement: string }> = [
    ...
    {
      pattern: /пространственн(ая|ый|ое|ые|ую|ых|ым)/gi,
      replacement: "простр.",
    },
...
export default function App() {
  ...
  const handleSaveData = () => {
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(clusters, null, 2));
    const dt = new Date();
  ...
  const handleClusterResizeStart = (
    cluster: ClusterData,
    e: React.MouseEvent,
  ) => {
    if (!isMoveMode) return;
  ...
  const [perfDisableBlur, setPerfDisableBlur] = useState<boolean>(
    () => localStorage.getItem("perf-blur-v1") !== "false",
  );
  const [perfDisableShadows, setPerfDisableShadows] = useState<boolean>(
    () => localStorage.getItem("perf-shadows-v1") !== "false",
  );
  const [perfDisableBg, setPerfDisableBg] = useState<boolean>(
    () => localStorage.getItem("perf-bg-v1") !== "false",
  );
  const [perfDisableAnimation, setPerfDisableAnimation] = useState<boolean>(
    () => localStorage.getItem("perf-anim-v1") !== "false",
  );
  ...
  const [isSphereSizeQuickOpen, setIsSphereSizeQuickOpen] =
  ...
  const togglePanelLock = (panelId: string) => {
    setPanelLocks((prev) => {
      const next = {
        ...prev,
        [panelId]: prev[panelId] === undefined ? false : !prev[panelId],
      };
  ...
  const areAllPanelsUnlocked = HUD_PANEL_IDS.every(
    (panelId) => !isPanelLocked(panelId),
  );

  const setAllPanelLocks = (locked: boolean) => {
    setPanelLocks((prev) => {
      const next = { ...prev };
  ...
  const getUnlockedPanelHighlight = (panelId: string) =>
    !isPanelLocked(panelId)
      ? "border-emerald-500/45 ring-2 ring-emerald-500/60 shadow-[0_0_26px_rgba(16,185,129,0.24)]"
      : "";
  ...
  const renderPanelLock = (panelId: string) => {
    ...
    // Замок выглядит как верхнее "ушко" панели.
    let positionClass = "top-[-27px] right-4 rounded-t-lg border-x border-t";
    if (panelId === "catalog" || panelId === "tophud") {
      positionClass = "top-[-27px] left-4 rounded-t-lg border-x border-t";
    }
    ...
    return (
      <button
        ...
        className={`absolute ${positionClass} w-8 h-7 flex items-center justify-center transition-all z-[100] pointer-events-auto cursor-pointer shadow-lg backdrop-blur-md ${isLocked ? "bg-[#070716]/95 border-white/10 text-slate-500 hover:text-amber-400 hover:border-amber-500/40" : "bg-emerald-950/95 border-emerald-500/60 text-emerald-300 shadow-[0_0_16px_rgba(16,185,129,0.35)]"}`}
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "archive-sphere-radius-scale-v5",
      sphereRadiusScale.toString(),
    );
    localStorage.setItem(
      "archive-sphere-position-scale-v5",
      spherePositionScale.toString(),
    );
    localStorage.setItem(
      "archive-topics-radius-scale-v1",
      topicsRadiusScale.toString(),
    );
    localStorage.setItem(
      "archive-flight-speed-mult-v5",
      flightSpeedMult.toString(),
    );
    ...
    localStorage.setItem(
      "archive-spheres-color-hue-v5",
      spheresColorHue.toString(),
    );
    localStorage.setItem("archive-custom-color-hex-v5", customColorHex);
    localStorage.setItem(
      "archive-physics-enabled-v5",
      isPhysicsEnabled.toString(),
    );
    ...
    localStorage.setItem(
      "archive-physics-friction-v5",
      physicsFriction.toString(),
    );
    ...
    localStorage.setItem(
      "archive-bg-image-opacity-v1",
      bgImageOpacity.toString(),
    );
    ...
    localStorage.setItem(
      "archive-sphere-border-width-v1",
      sphereBorderWidth.toString(),
    );
    localStorage.setItem("archive-sphere-glow-v1", sphereGlow.toString());
    localStorage.setItem(
      "archive-sphere-roundness-v1",
      sphereRoundness.toString(),
    );
    localStorage.setItem(
      "archive-sphere-custom-clippath-v1",
      customShapeClipPath,
    );
  }, [
    flightSpeedMult,
  ...
  const colorThemes = [
    { id: "default", name: "Архивы", label: "Оригинал" },
    {
      ...
      name: "Акаша",
      label: "Акаша Пурпур",
      gradient: "linear-gradient(135deg, #7028e4 0%, #30cfd0 100%)",
    },
    {
      id: "crimson_gold",
      name: "Вспышка",
  ...
  const shapePresets = [
    ...
    },
    {
      ...
      name: "Сердце",
      clipPath:
        'path("M50,30 C50,20 35,10 25,20 C12,32 25,50 50,72 C75,50 88,32 75,20 C65,10 50,20 50,30 Z")',
    },
  ];
  ...
  const generateClipPathFromDescription = (desc: string): string | null => {
    ...
    if (d.includes("треуг") || d.includes("triangle") || d.includes("пирамид"))
      return genPolygon(3);
    if (d.includes("ромб") || d.includes("diamond") || d.includes("алмаз"))
      return genPolygon(4, -90);
    ...
    if (d.includes("восьмиуг") || d.includes("octa") || d.includes("октаг"))
      return genPolygon(8);
  ...
  const getShapeStyle = (
    ...
    const preset =
      shapePresets.find((p) => p.id === sphereShape) || shapePresets[0];
  ...
  const applyShapeDescription = () => {
    ...
    window.alert(
      "Не удалось распознать форму. Примеры: «круг», «квадрат», «эллипс», «треугольник», «7-угольник», «звезда 9 лучей», «12 сторон», «сердце».",
    );
  };
  ...
  const [isCatalogHidden, setIsCatalogHidden] = useState<boolean>(false);
  ...
  useEffect(() => {
    localStorage.setItem(
      "archive-decryptor-hidden-v5",
      isDecryptorHidden.toString(),
    );
  }, [isDecryptorHidden]);
  useEffect(() => {
  ...
  useEffect(() => {
    localStorage.setItem(
      "archive-catalog-hidden-v5",
      isCatalogHidden.toString(),
    );
  }, [isCatalogHidden]);

  const [shouldAnimate, setShouldAnimate] = useState<boolean>(false);
  const animationTimeoutRef = useRef<any>(null);
  const windowMousePosRef = useRef<{ x: number; y: number }>({
    x: -1000,
    y: -1000,
  });
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      if (
        settingsRef.current?.isMoveMode ||
        settingsRef.current?.perfDisableAnimation
      ) {
      ...
      if (nodes.length > 0) {
        for (let i = 0; i < nodes.length; i++) {
          const item = nodes[i];
          const currentAngRad =
            (item.startAng + globalT * item.dir) * (Math.PI / 180);
      ...
      const currentNodes =
        document.querySelectorAll<HTMLElement>(".anim-orbit-node");
      if (
        currentNodes.length !== lastNodeCount ||
        !cachedNodes ||
        Math.random() < 0.02
      ) {
        cachedNodes = Array.from(currentNodes).map((node) => {
          ...
          const startAng = parseFloat(
            node.getAttribute("data-start-ang") || "0",
          );
          ...
          const lineEl =
            lineId !== "none"
              ? (document.getElementById(
                  lineId,
                ) as unknown as SVGLineElement | null)
              : null;
          return {
            node,
            cx,
  ...
  const TooltipLayer = React.memo(
    ({ definitions }: { definitions: Record<string, string> }) => {
          ...
          {hoveredTerm && (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="font-display font-bold text-sm text-indigo-300">
                  {hoveredTerm}
                </h4>
              </div>
  ...
  const [isViewingClusterInfo, setIsViewingClusterInfo] =
    useState<boolean>(false);
  const [isEditingClusterDef, setIsEditingClusterDef] =
    useState<boolean>(false);
  ...
  const resizeStartRef = useRef<{ w: number; h: number; x: number; y: number }>(
    { w: 550, h: 650, x: 0, y: 0 },
  );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
      ...
      const newW = Math.max(
        320,
        Math.min(window.innerWidth - 30, resizeStartRef.current.w + deltaX),
      );
      const newH = Math.max(
        300,
        Math.min(window.innerHeight - 30, resizeStartRef.current.h + deltaY),
      );
  ...
  const [localClusterDefs, setLocalClusterDefs] = useState<
  ...
  const updateClusterDef = (clusterId: string, text: string) => {
    setLocalClusterDefs((prev) => {
      ...
      localStorage.setItem(
        "archive-custom-cluster-defs-v5",
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (resizedClusterId) {
        ...
        updateClusterPositionAndRadius(
          resizedClusterId,
          editStartRef.current.cX,
          editStartRef.current.cY,
          nextR,
        );
        return;
      }
  ...
  const getVisibility = (zoomLevel: number, threshold: number) =>
    zoomLevel >= threshold;

  // Polar coordinate mapping formulas
  const getItemDistance = (
    cluster: ClusterData,
    item: { isInner?: boolean },
  ) => {
  ...
  const flyToPosition = useCallback(
    (pos: { x: number; y: number }, targetZoom: number) => {
      if (animationTimeoutRef.current) {
        clearTimeout(animationTimeoutRef.current);
      }
  ...
  const findTermTarget = useCallback(
    (term: string): TermTarget | null => {
      ...
      for (const cluster of clusters) {
        if (
          ...
          return {
            pos: {
              x: cluster.x * spherePositionScale,
              y: cluster.y * spherePositionScale,
            },
            zoomLevel: 1.15,
          };
        }
  ...
  const handleTermClick = (termText: string) => {
    ...
    setEditingText(
      definitions[text] ||
        "Определение для данного термина пока отсутствует в архивах.",
    );
    setIsEditingDesc(false);
  };
  ...
  const handleTermHover = (termText: string, e: React.MouseEvent) => {
    ...
    window.dispatchEvent(
      new CustomEvent("term-hover", {
        detail: { term: termText, x: e.clientX, y: e.clientY },
      }),
    );
  };
  ...
  const handleClusterClick = (cluster: ClusterData) => {
    if (activeCluster?.id === cluster.id) {
      debugLog({
        type: "event",
        message: `Sphere deactivated: "${cluster.title}"`,
      });
      setActiveCluster(null);
      setIsViewingClusterInfo(false);
    } else {
      debugLog({
        type: "event",
        message: `Sphere activated: "${cluster.title}"`,
        detail: `id=${cluster.id}`,
      });
  ...
  const filteredClusters = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = clusters.filter((cluster) => {
        ...
        const subtopicMatch = cluster.subtopics.some((s) =>
          s.text.toLowerCase().includes(query),
        );
        const detailsMatch = cluster.details.some((d) =>
  ...
  return (
    <div
      ...
      className="relative w-screen h-screen bg-[#02020a] overflow-hidden select-none font-sans text-slate-100"
    >
      ...
      {!perfDisableBg && (
        <>
          ...
          />
          <div className="absolute inset-x-0 top-0 h-40 pointer-events-none bg-gradient-to-b from-indigo-950/20 to-transparent" />
          <div className="absolute inset-y-0 right-0 w-96 pointer-events-none bg-gradient-to-l from-[#0d0925]/40 to-transparent" />
        </>
      )}
              ...
              {filteredClusters.flatMap((cluster, clusterIdx) => {
                ...
                cluster.subtopics.forEach((sub, i) => {
                  ...
                  if (cluster.id === "q-anon" || !sub.isInner) {
                    ...
                    lines.push(
                      <line
                        key={`sub-line-${cluster.id}-${i}`}
                        id={`line-sub-${cluster.id}-${i}`}
                        x1={x1}
                        y1={y1}
                        x2={x2}
                        y2={y2}
                        ...
                        strokeOpacity="0.4"
                        strokeDasharray="4 4"
                        className="transition-opacity duration-300 peer-hover/sub:opacity-100"
                      />,
                    );
                  }
                });
                ...
                cluster.details.forEach((det, i) => {
                  ...
                  if (cluster.id === "q-anon" || !det.isInner) {
                    ...
                    lines.push(
                      <line
                        key={`det-line-${cluster.id}-${i}`}
                        id={`line-det-${cluster.id}-${i}`}
                        x1={x1}
                        y1={y1}
                        x2={x2}
                        y2={y2}
                        ...
                        strokeOpacity="0.2"
                        strokeDasharray="2 2"
                        className="transition-opacity duration-300 peer-hover/det:opacity-100"
                      />,
                    );
                  }
                });
              ...
              <React.Fragment key={cluster.id}>
                ...
                {isActive && !isMoveMode && !perfDisableShadows && (
                  <motion.div
                    ...
                    style={{
                      ...
                      boxShadow:
                        "0 0 45px rgba(6, 182, 212, 0.5), inset 0 0 20px rgba(6, 182, 212, 0.3)",
                      filter:
                        sphereBaseColor === "default" && spheresColorHue !== 0
                          ? `hue-rotate(${spheresColorHue}deg)`
                          : undefined,
                    }}
                    animate={{
                ...
                <div
                  className={`cluster absolute flex flex-col items-center justify-center text-center font-display font-semibold border-2 select-none z-10 ${
                    isFadedOut
                      ? "opacity-10 grayscale hover:opacity-50 transition-opacity"
                      : getVisibility(zoom, 0.3)
                        ? "opacity-100"
                        : "opacity-60 scale-95"
                  } ${
                    isMoveMode
                  ...
                  style={{
                    ...
                    boxShadow:
                      !perfDisableShadows && sphereGlow > 0
                  ...
                  onMouseDown={
                    isMoveMode
                      ? (e) => handleClusterDragStart(cluster, e)
                      : undefined
                  }
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                {cluster.subtopics.map((subtopic, idx) => {
                  ...
                  const colors = getSubtopicColors(
                    subtopic.text,
                    solidColor,
                    subtopic.color,
                  );
                  ...
                  const isActiveCluster =
                    activeCluster && activeCluster.id === cluster.id;
                  ...
                  const rad = (subtopic.angle * Math.PI) / 180;
                  const targetX =
                    cluster.x * spherePositionScale +
                    Math.cos(rad) *
                      scaledDistance *
                      (cluster.shapeRatioX || 1.0);
                  const targetY =
                    cluster.y * spherePositionScale +
                    Math.sin(rad) *
                      scaledDistance *
                      (cluster.shapeRatioY ||
                        (cluster.id === "q-anon" ? 0.75 : 1.0));
                  ...
                  return (
                    <div
                      ...
                      data-line-id={
                        isOutside ? `line-sub-${cluster.id}-${idx}` : "none"
                      }
                    ...
                    >
                      <div
                        className="relative z-10 group/sub hover:z-50 transition-all duration-300"
                        style={{
                          transform: `translate(-50%, -50%) scale(${subtopicScale})`,
                        }}
                      >
                        <div
                          ...
                          style={{
                            ...
                            padding: "4px 10px",
                            borderRadius: "8px",
                            backdropFilter: perfDisableBlur
                              ? "none"
                              : "blur(2px)",
                            boxShadow: perfDisableShadows
                              ? "none"
                              : "0 4px 12px rgba(0,0,0,0.5)",
                            color: colors.textColor,
                            WebkitTextStroke: perfDisableShadows
                              ? "none"
                              : "0.2px rgba(0,0,0,0.5)",
                            textShadow: perfDisableShadows
                              ? "none"
                          ...
                          onMouseEnter={(e) =>
                            handleTermHover(subtopic.text, e)
                          }
                ...
                {cluster.details.map((detail, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`absolute select-none z-20 hover:z-[100] anim-orbit-node`}
                      data-line-id={
                        isOutside ? `line-det-${cluster.id}-${idx}` : "none"
                      }
                      data-radius={scaledRadius}
                      data-cx={cluster.x * spherePositionScale}
                      data-cy={cluster.y * spherePositionScale}
                    ...
                    >
                      <div
                        className="relative z-10 group/det hover:z-50 transition-all duration-300"
                        style={{
                          transform: `translate(-50%, -50%) scale(${detailScale})`,
                        }}
                      >
                        <div
                          ...
                          style={{
                            ...
                            pointerEvents: detailPointerEvents as
                              | "auto"
                              | "none",
      ...
      <AnimatePresence>
        {true && (
          <motion.div
            ...
            style={{
              transformOrigin: "top left",
              ...getDragProps("tophud").style,
            }}
            {...getDragProps("tophud")}
            className={`absolute top-[-42px] left-[-2px] flex flex-col items-start gap-1.5 z-50 max-h-[calc(100vh-36px)] overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${getUnlockedPanelHighlight("tophud")} ${!isPanelLocked("tophud") ? "rounded-xl bg-emerald-950/20" : ""}`}
          >
            ...
            <button
              ...
              title={
                isMoveMode ? "Выход из перемещения" : "Включить перемещение"
              }
            >
            ...
            <button
              onClick={() => setAllPanelLocks(areAllPanelsUnlocked)}
              className={`hud-panel pointer-events-auto hover:bg-slate-900 border transition-all duration-200 w-9 rounded-lg backdrop-blur-xl shadow-[0_10px_25px_rgba(0,0,0,0.4)] flex items-center justify-center h-9 shrink-0 cursor-pointer ${
                areAllPanelsUnlocked
                  ? "bg-emerald-950/40 border-emerald-500/60 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.25)]"
                  : "bg-[#070716]/80 border-indigo-500/20 text-indigo-300 hover:text-white"
              }`}
              title={
                areAllPanelsUnlocked
                  ? "Заблокировать панели UI"
                  : "Разблокировать позицию панелей UI"
              }
            >
          ...
          {!isDecryptorHidden && (
            <motion.div
              ...
              style={{
                ...
                ...getDragProps("decryptor").style,
              }}
              {...getDragProps("decryptor")}
              className={`hud-panel absolute top-[-20px] right-[-35px] w-64 max-h-screen bg-[#070716]/85 backdrop-blur-xl border border-white/5 p-3 rounded-xl shadow-[0_15px_35px_rgba(0,0,0,0.6)] z-30 flex flex-col gap-2.5 overflow-y-auto custom-scrollbar prevent-wheel-zoom ${getUnlockedPanelHighlight("decryptor")}`}
            >
              {renderPanelLock("decryptor")}
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                <div className="flex items-center gap-2">
                  <Compass className="w-4 h-4 text-cyan-400" />
                  <h2 className="font-mono font-medium text-[9px] tracking-widest text-slate-400 uppercase">
                    ДЕКРИПТОР СФЕР
                  </h2>
                </div>
                <button
              ...
              {activeCluster ? (
                <div className="flex flex-col gap-3">
                  ...
                  <p className="text-xs text-slate-300/90 leading-relaxed font-sans line-clamp-2">
                    {localClusterDefs[activeCluster.id] ||
                      "Нет данных для считывания."}
                  </p>
                  ...
                  <div className="flex gap-1.5 w-full pt-0.5">
                    <button
                      onClick={() =>
                        flyToPosition(
                          { x: activeCluster.x, y: activeCluster.y },
                          1.15,
                        )
                      }
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        updateClusterPositionAndRadius(
                          ...
                          activeCluster.y,
                          val,
                          activeCluster.shapeRatioX,
                          activeCluster.shapeRatioY,
                        );
                        setActiveCluster((prev) =>
                          prev ? { ...prev, radius: val } : prev,
                        );
                      }}
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      <div className="flex flex-col gap-1">
                        <span className="text-[9px] font-mono text-slate-500 tracking-wider">
                          Сжатие X:{" "}
                          {(activeCluster.shapeRatioX || 1.0).toFixed(2)}
                        </span>
                        <input
                          type="range"
                          min="0.3"
                          ...
                          onChange={(e) => {
                            ...
                            setActiveCluster((prev) =>
                              prev ? { ...prev, shapeRatioX: val } : prev,
                            );
                          }}
                      ...
                      <div className="flex flex-col gap-1">
                        <span className="text-[9px] font-mono text-slate-500 tracking-wider">
                          Сжатие Y:{" "}
                          {(activeCluster.shapeRatioY || 1.0).toFixed(2)}
                        </span>
                        <input
                          type="range"
                          min="0.3"
                          ...
                          onChange={(e) => {
                            ...
                            setActiveCluster((prev) =>
                              prev ? { ...prev, shapeRatioY: val } : prev,
                            );
                          }}
                          className="w-full accent-indigo-400 h-1 bg-white/5 rounded-lg appearance-none cursor-pointer"
                        />
          ...
          {isSettingsOpen && (
            <motion.div
              ...
              style={{
                ...
                ...getDragProps("parametris_efir").style,
              }}
              {...getDragProps("parametris_efir")}
              className={`hud-panel absolute bottom-4 right-4 w-64 bg-[#070716]/92 backdrop-blur-2xl border border-cyan-500/20 p-3.5 rounded-xl shadow-[0_25px_50px_rgba(0,0,0,0.85)] z-30 flex flex-col gap-2.5 max-h-[calc(100vh-100px)] overflow-y-auto custom-scrollbar prevent-wheel-zoom ${getUnlockedPanelHighlight("parametris_efir")}`}
            >
              {renderPanelLock("parametris_efir")}
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                <div className="flex items-center gap-1.5">
                  <Sliders className="w-3.5 h-3.5 text-cyan-400" />
                  <h2 className="font-mono font-semibold text-[9px] tracking-widest text-slate-300 uppercase">
                    ПАРАМЕТРЫ ЭФИРА
                  </h2>
                </div>
                <button
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFlightSpeedMult(parseFloat(e.target.value))
                  }
                  className="w-full accent-cyan-400 h-1 bg-white/5 rounded-lg appearance-none cursor-pointer"
                />
              </div>
              ...
              {sphereBaseColor === "default" && (
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSpheresColorHue(parseInt(e.target.value))
                    }
                    className="w-full accent-cyan-400 h-1 bg-white/5 rounded-lg appearance-none cursor-pointer"
                  />
                </div>
        ...
        <AnimatePresence>
          {isMapSettingsOpen && (
            <motion.div
              ...
              style={{
                transformOrigin: "top right",
                ...getDragProps("map").style,
              }}
              {...getDragProps("map")}
              className={`hud-panel absolute top-[120px] right-4 w-64 bg-[#070716]/92 backdrop-blur-2xl border border-indigo-500/20 p-3.5 rounded-xl shadow-[0_25px_50px_rgba(0,0,0,0.85)] z-30 flex flex-col gap-2.5 max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar prevent-wheel-zoom ${getUnlockedPanelHighlight("map")}`}
            >
              {renderPanelLock("map")}
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                <div className="flex items-center gap-1.5">
        ...
        <AnimatePresence>
          {isSizeSettingsOpen && (
            <motion.div
              ...
              style={{
                transformOrigin: "top right",
                ...getDragProps("size").style,
              }}
              {...getDragProps("size")}
              className={`hud-panel absolute top-[120px] right-4 w-64 bg-[#070716]/92 backdrop-blur-2xl border border-indigo-500/20 p-3.5 rounded-xl shadow-[0_25px_50px_rgba(0,0,0,0.85)] z-30 flex flex-col gap-2.5 max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar prevent-wheel-zoom ${getUnlockedPanelHighlight("size")}`}
            >
              ...
              <div className="flex flex-col gap-2.5 items-center justify-center py-6">
                <span className="text-slate-500 text-xs font-mono">
                  Настройки размера (в разработке)
                </span>
              </div>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {isSphereSizeQuickOpen && (
            <motion.div
              ...
              style={{
                transformOrigin: "top right",
                ...getDragProps("spheresize").style,
              }}
              {...getDragProps("spheresize")}
              className={`hud-panel absolute top-[120px] right-4 w-64 bg-[#070716]/92 backdrop-blur-2xl border border-indigo-500/20 p-3.5 rounded-xl shadow-[0_25px_50px_rgba(0,0,0,0.85)] z-30 flex flex-col gap-3 max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar prevent-wheel-zoom ${getUnlockedPanelHighlight("spheresize")}`}
            >
              ...
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setIsSphereSizeQuickOpen(false);
                  }}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSphereRadiusScale(parseFloat(e.target.value))
                  }
                  className="w-full accent-cyan-400 h-1 bg-white/5 rounded-lg appearance-none cursor-pointer"
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSphereBorderWidth(parseFloat(e.target.value))
                  }
                  className="w-full accent-cyan-400 h-1 bg-white/5 rounded-lg appearance-none cursor-pointer"
                />
              </div>
              ...
              {(sphereShape === "circle" || sphereShape === "ellipse") && (
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSphereRoundness(parseFloat(e.target.value))
                    }
                    className="w-full accent-cyan-400 h-1 bg-white/5 rounded-lg appearance-none cursor-pointer"
                  />
                </div>
              ...
              <div className="flex flex-col gap-1.5 border-t border-white/5 pt-2.5">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") applyShapeDescription();
                    }}
                ...
                <p className="text-[8px] text-slate-500 leading-relaxed font-mono">
                  Любой многоугольник или звезда строятся по координатам:
                  укажите число сторон/лучей.
                </p>
              </div>
        ...
        <AnimatePresence>
          {isPerfSettingsOpen && (
            <motion.div
              ...
              style={{
                transformOrigin: "top right",
                ...getDragProps("perf").style,
              }}
              {...getDragProps("perf")}
              className={`hud-panel absolute top-[120px] right-4 w-64 bg-[#070716]/92 backdrop-blur-2xl border border-indigo-500/20 p-3.5 rounded-xl shadow-[0_25px_50px_rgba(0,0,0,0.85)] z-30 flex flex-col gap-2.5 max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar prevent-wheel-zoom ${getUnlockedPanelHighlight("perf")}`}
            >
              ...
              <div className="flex flex-col gap-1.5 pt-1">
                ...
                <div className="grid grid-cols-2 gap-1.5 mt-1">
                  ...
                  >
                    ...
                    <div
                      className={`w-3.5 h-3.5 rounded-full border flex-shrink-0 ${perfDisableBlur ? "border-amber-400 bg-amber-400/20" : "border-slate-500 bg-transparent"}`}
                    />
                  </button>
                  ...
                  >
                    ...
                    <div
                      className={`w-3.5 h-3.5 rounded-full border flex-shrink-0 ${perfDisableShadows ? "border-amber-400 bg-amber-400/20" : "border-slate-500 bg-transparent"}`}
                    />
                  </button>
                  ...
                  >
                    ...
                    <div
                      className={`w-3.5 h-3.5 rounded-full border flex-shrink-0 ${perfDisableBg ? "border-amber-400 bg-amber-400/20" : "border-slate-500 bg-transparent"}`}
                    />
                  </button>

                  <button
                    onClick={() =>
                      setPerfDisableAnimation(!perfDisableAnimation)
                    }
                    className={`px-2 py-1.5 rounded-xl text-[10px] font-semibold border transition-all flex items-center justify-between cursor-pointer ${
                      perfDisableAnimation
                        ? "bg-amber-950/20 text-amber-400 border-amber-500/30"
                  ...
                  >
                    ...
                    <div
                      className={`w-3.5 h-3.5 rounded-full border flex-shrink-0 ${perfDisableAnimation ? "border-amber-400 bg-amber-400/20" : "border-slate-500 bg-transparent"}`}
                    />
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5 border-t border-white/5 pt-2.5 mt-1">
                ...
                <div className="grid grid-cols-2 gap-1.5 mt-1">
                  ...
                  >
                    ...
                    {!isLegendHidden ? (
                      <HelpCircle className="w-3.5 h-3.5 text-cyan-300 flex-shrink-0" />
                    ) : (
                      <EyeOff className="w-3.5 h-3.5 text-slate-500 flex-shrink-0" />
                    )}
                  </button>
                  ...
                  <div className="flex flex-col gap-2 mt-2">
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        value={bgImageScale}
                        onChange={(e) =>
                          setBgImageScale(parseFloat(e.target.value))
                        }
                        className="w-full accent-indigo-400 h-1 bg-white/5 rounded-lg appearance-none cursor-pointer"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex flex-col gap-1">
                        ...
                        <input
                          ...
                          value={bgImageX}
                          onChange={(e) =>
                            setBgImageX(parseFloat(e.target.value) || 0)
                          }
                          className="bg-slate-900 border border-slate-700 rounded-md p-1 pl-2 text-xs text-slate-300 focus:outline-none focus:border-indigo-500"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-[10px] text-slate-400">
                        ...
                        <input
                          ...
                          value={bgImageY}
                          onChange={(e) =>
                            setBgImageY(parseFloat(e.target.value) || 0)
                          }
                          className="bg-slate-900 border border-slate-700 rounded-md p-1 pl-2 text-xs text-slate-300 focus:outline-none focus:border-indigo-500"
                        />
                      </div>
        ...
        <AnimatePresence>
          {!isLegendHidden && (
            <motion.div
              ...
              style={{
                transformOrigin: "bottom right",
                ...getDragProps("navigator").style,
              }}
              {...getDragProps("navigator")}
              className={`hud-panel absolute bottom-[-45px] right-[-35px] w-60 bg-[#070716]/85 backdrop-blur-xl border border-white/5 p-3.5 rounded-xl shadow-[0_20px_45px_rgba(0,0,0,0.7)] z-20 ${getUnlockedPanelHighlight("navigator")}`}
            >
              {renderPanelLock("navigator")}
              <div className="flex items-center justify-between mb-2 border-b border-white/5 pb-2">
                <div className="flex items-center gap-1.5">
                  <Compass className="w-4 h-4 text-purple-400" />
                  <h2 className="font-display font-medium text-xs text-slate-200 tracking-wider">
                    НАВИГАТОР АРХИВА
                  </h2>
                </div>
                <button
              ...
              <div className="space-y-2 text-xs text-slate-300/80 leading-relaxed font-sans">
                ...
                <p>
                  ...
                  — открыть полную расшифровку теории
                </p>
                <p>
                  •{" "}
                  <span className="font-semibold text-slate-100">
      ...
      <div
        className={`absolute inset-0 z-40 transition-opacity duration-300 pointer-events-none ${isUiHidden ? "opacity-5" : "opacity-35"}`}
      >
          ...
          {!isCatalogHidden && (
            <motion.div
              ...
              style={{
                ...
                ...getDragProps("catalog").style,
              }}
              drag={getDragProps("catalog").drag}
              dragMomentum={false}
              dragElastic={0}
              dragTransition={{
                bounceStiffness: 0,
            ...
            >
              ...
              {activeCluster ? (
                <>
                  <div
                    className="flex items-center justify-between px-3 py-2 border-b border-white/5 flex-shrink-0 cursor-pointer hover:bg-white/5 transition-colors"
                  ...
                  {isCatalogOpen && (
                    <div className="overflow-y-auto flex-1 divide-y divide-white/5 py-1 prevent-wheel-zoom">
                      ...
                      {activeCluster.subtopics.length === 0 &&
                        activeCluster.details.length === 0 && (
              ...
              {isEditingClusterDef ? (
                ...
                <div>
                  ...
                  <div className="bg-[#020207] border border-cyan-500/10 rounded-xl p-5 leading-relaxed text-sm md:text-base text-slate-200 max-h-56 overflow-y-auto prevent-wheel-zoom">
                    {localClusterDefs[activeCluster.id] ||
                      "Сведения временно недоступн��."}
                  </div>

                  {/* Subtopics and details of this sphere as quick-jump tags */}
                  {(activeCluster.subtopics &&
                    activeCluster.subtopics.length > 0) ||
                  (activeCluster.details && activeCluster.details.length > 0) ||
                  isMoveMode ? (
                    <div className="mt-5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...(activeCluster.subtopics || []).map((s) => ({
                            ...s,
                            isDetail: false,
                          })),
                          ...(activeCluster.details || []).map((d) => ({
                            ...d,
                            isDetail: true,
                          })),
                        ].map((sub, idx) => (
                          <div
                            key={`sub-tag-${idx}`}
                            className="flex items-stretch gap-0 border border-indigo-500/20 rounded-lg overflow-hidden group"
                          >
                            {isMoveMode && (
                              <input
                                type="color"
                      ...
                      {isMoveMode && (
                        <div className="mt-4 flex flex-col gap-3 p-3 bg-indigo-950/20 rounded-lg border border-indigo-500/20">
                          <span className="text-indigo-300 text-[10px] font-mono uppercase tracking-widest">
                            ФОРМА СФЕРЫ:
                          </span>
                          <div className="flex flex-col gap-3 shadow-inner">
                            <label className="flex flex-col gap-1.5">
                              <span className="text-slate-400 text-xs font-semibold">
                                Радиус: {activeCluster.radius}
                              </span>
                              <input
                                type="range"
                                min="20"
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <label className="flex flex-col gap-1.5">
                                <span className="text-slate-400 text-xs font-semibold">
                                  Сжатие X: {activeCluster.shapeRatioX || 1.0}
                                </span>
                                <input
                                  type="range"
                                  min="0.1"
                              ...
                              <label className="flex flex-col gap-1.5">
                                <span className="text-slate-400 text-xs font-semibold">
                                  Сжатие Y: {activeCluster.shapeRatioY || 1.0}
                                </span>
                                <input
                                  type="range"
                                  min="0.1"
                            ...
                            <label className="flex flex-col gap-1.5 mt-2">
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  if (val) {
                                    activeCluster.subtopics?.forEach((s) =>
                                      updateSubtopicColor(
                                        activeCluster.id,
                                        s.text,
                                        val,
                                    ...
                                    activeCluster.details?.forEach((d) =>
                                      updateSubtopicColor(
                                        activeCluster.id,
                                        d.text,
                                        val,
                      ...
                      {isMoveMode && (
                        <div className="mt-3 p-3 bg-slate-900 rounded-lg border border-slate-700">
                          ...
                          <div className="mt-4 pt-4 border-t border-indigo-500/20">
                            <span className="text-indigo-300 text-[10px] font-mono uppercase tracking-widest block mb-2">
                              ПОСТРОЕНИЕ СВЯЗЕЙ:
                            </span>
                          ...
                          <div className="mt-4 pt-4 border-t border-red-500/20">
                            <button
                              onClick={() => {
                                if (
                                  confirm(
                                    "Вы уверены, что хотите удалить эту сферу?",
                                  )
                                ) {
              ...
              <button
                onClick={() => {
                  ...
                  if (isEditingClusterDef && activeCluster) {
                    ...
                    localStorage.setItem(
                      "archive-custom-cluster-defs-v5",
                      JSON.stringify(updatedClusterDefs),
                    );
                  }
                }}
              ...
              <div className="px-6 py-6 pb-12 space-y-8">
                ...
                <section>
                  ...
                  <div className="text-sm text-slate-300 leading-relaxed space-y-3">
                    <p>
                      <strong>Архив Логоса</strong> — это передовая визуальная
                      нода для исследования и структурирования сложных
                      эзотерических, исторических и конспирологических данных.
                      Проект объединяет разрозненные элементы в единое
                      глобальное древо знаний.
                    </p>
                    <p>
                      Постр����н на базе современного стека технологий, что
                      обеспечивает плавную симуляцию косми��еской среды:
                      гравитации кластеров, динамических орбит подтем и плавных
                      переходов камеры.
                    </p>
                  </div>
                </section>
                ...
                {/* Tech Stack */}
                <section>
                  <h4 className="text-xs font-mono font-bold text-indigo-400 uppercase tracking-widest mb-3 border-b border-white/10 pb-2">
                    Стек Технологий
                  </h4>
                    ...
                    <li className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-slate-400">
                          Производительный интерфейс и сборка
                        </span>
                      </div>
                    </li>
                    <li className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-slate-400">
                          Плавная анимация окон, списков и элементов HUD
                        </span>
                      </div>
                    </li>
                    <li className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-slate-400">
                          Утилитарная стилизация, эффекты стекла и теней
                        </span>
                      </div>
                    </li>
                    <li className="flex items-start gap-2">
                ...
                {/* Features */}
                <section>
                  <h4 className="text-xs font-mono font-bold text-cyan-400 uppercase tracking-widest mb-3 border-b border-white/10 pb-2">
                    Основные возможности
                  </h4>
                  <ul className="space-y-4 text-sm text-slate-300">
                    <li className="flex flex-col gap-1">
                      <strong className="text-white">
                        Движок Планетарной Физики (Cosmic Engine)
                      </strong>
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Реализует базовые законы физики: гравитационное
                        притяжение между сферами (основанное на их
                        массе/радиусе), трение космического эфира и упругое
                        столкновение объектов. Сферы имеют склонность
                        возвращаться в свои узлы (позиции) для поддержания
                        структуры созвездий.
                      </p>
                    </li>
                    <li className="flex flex-col gap-1">
                      <strong className="text-white">
                        Бесконечный Canvas с ZUI
                      </strong>
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Слоистый интерфейс ZUI (Zoomable User Interface) плавно
                        скрывает элементы информации в зависимости от
                        масштабирования (уровня погружения слоя), помогая
                        сохранить визу��льную чистоту карты и фокусировку на
                    ...
                    <li className="flex flex-col gap-1">
                      ...
                      <p className="text-xs text-slate-400 leading-relaxed">
                        Пользователи могут менять размер сфер, орбиты, скорость
                        полета между кластерами и активировать уникальные
                        цветовые схемы (например, «Космический Свет»,
                        «Кибер-Розовый», и другие) для персонализации
                        пространства.
                      </p>
                    </li>
                    <li className="flex flex-col gap-1">
                      <strong className="text-white">
                        Режим перемещения сущностей
                      </strong>
                      <p className="text-xs text-slate-400 leading-relaxed">
                        В этом режиме гравитация отключается, а
                        пространственно-временной континуум фиксируется,
                        позволяя пользователю вручную перетаскивать планетарные
                        кластеры в новые точки и редактировать их размер.
                      </p>
                    </li>
                  </ul>
                </section>
      ...
      <AnimatePresence>
        {showLoadToast && (
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="font-mono text-xs text-emerald-400 tracking-widest uppercase">
                Система л��ггирования
              </span>
              <span className="font-sans font-medium text-white text-sm">
                Данные Архива успешно загружены!
              </span>
            </div>
          </motion.div>
        )}
