  ...
  const renderPanelLock = (panelId: string) => {
    ...
    return (
      <button 
        ...
        className={`w-fit self-start p-0.5 rounded transition-colors flex-shrink-0 ${isLocked ? 'text-slate-600 hover:text-amber-400' : 'text-amber-400 hover:text-slate-500'}`}
...
<div className={`absolute inset-0 z-40 transition-opacity duration-300 pointer-events-none ${isUiHidden ? 'opacity-5' : 'opacity-35'}`}>
      ...
      <AnimatePresence>
        {!isCatalogHidden && (
          <motion.div
            ...
            className={`hud-panel absolute bottom-[6px] left-0 w-64 max-h-[calc(100vh-80px)] overflow-hidden flex flex-col bg-[#070716]/50 backdrop-blur-xl border border-white/5 rounded-xl shadow-[0_20px_45px_rgba(0,0,0,0.8)] z-40 ${isCatalogOpen ? 'h-[320px]' : 'h-[40px]'} pointer-events-auto ${isPanelsUnlocked || isCatalogMovable ? 'ring-2 ring-emerald-500/50' : ''} ${isPanelBeingDragged('catalog') ? '' : 'transition-all duration-300'} z-40 pointer-events-auto ${isPanelsUnlocked || isCatalogMovable ? 'ring-2 ring-emerald-500/50' : ''}`}
          >
